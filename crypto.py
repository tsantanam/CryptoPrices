...
def index():
    if 'button1' in request.form:
        ...
        if len(i) > 0:
            ...
            return render_template('index.html', price=round(float(price),2), change = round(float(change_24h),2), spread = round(float(spread),2), volume = round(float(volume_24h),2))
