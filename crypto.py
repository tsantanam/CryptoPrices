...
def index():
    if 'button1' in request.form:
        ...
        if len(i) > 0:
            ...
            return render_template('index.html', asset = asset, price=round(float(price),4), change = round(float(change_24h),4), spread = round(float(spread),4), volume = round(float(volume_24h),4))
